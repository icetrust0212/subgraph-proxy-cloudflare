...
const cacheStore = (env: Env, ttl: number) => {
  return {
    async get(key: string) {
      const data = await env.ryze_proxy_test.get<CacheData>(key, "json");
    ...
    async set(key: string, data: CacheData) {
      await env.ryze_proxy_test.put(key, JSON.stringify(data), {
        expirationTtl: ttl,
      });
    },
  };
      ...
      switch (serviceType) {
        case "gateway":
          return getGatewayUrl({
            apiKey: args.contextValue.env.SUBGRAPH_API_KEY || identifier,
            subgraphId: name,
          });
